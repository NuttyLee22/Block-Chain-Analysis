...
--
-- Temporary view structure for view `kpis`
--

DROP TABLE IF EXISTS `kpis`;
/*!50001 DROP VIEW IF EXISTS `kpis`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `kpis` AS SELECT 
 1 AS `Pedido Numero`,
 1 AS `Operacao`,
 1 AS `Uso`,
 1 AS `Transportadora`,
 ...
 1 AS `Data Entregue`,
 1 AS `Tempo Entrega`,
 1 AS `Tempo Entrega Dias`,
 1 AS `Ciclo Pedido`,
 1 AS `Ciclo Pedido Dias`,
 1 AS `Cliente`,
 1 AS `Pedidos Recebidos OTIF`,
 1 AS `Pedidos Recebidos % OTIF`,
 1 AS `Total Pedidos OTIF`,
 1 AS `Pedidos Recebidos OTD`,
 1 AS `Pedidos Recebidos % OTD`,
 1 AS `Total Pedidos OTD`,
 1 AS `Total Pedidos OFR`,
 1 AS `Pedidos Atendidos Completamente OFR`,
 1 AS `Order Fill Rate OFR`*/;
SET character_set_client = @saved_cs_client;
...
--
-- Final view structure for view `kpis`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `kpis` AS select `te`.`Pedido Numero` AS `Pedido Numero`,`te`.`Operacao` AS `Operacao`,`te`.`Uso` AS `Uso`,`te`.`Transportadora` AS `Transportadora`,`te`.`Grupo` AS `Grupo`,`te`.`Municipio` AS `Municipio`,`te`.`UF` AS `UF`,`te`.`Emitido em` AS `Emitido em`,`te`.`Previsao Entrega` AS `Previsao Entrega`,`te`.`Data Embarque` AS `Data Embarque`,`te`.`Data Entregue` AS `Data Entregue`,`te`.`Tempo Entrega` AS `Tempo Entrega`,`te`.`Dias` AS `Tempo Entrega Dias`,`tcp`.`Ciclo Pedido` AS `Ciclo Pedido`,`tcp`.`Dias` AS `Ciclo Pedido Dias`,`o`.`Cliente` AS `Cliente`,`o`.`Pedidos Recebidos` AS `Pedidos Recebidos OTIF`,`o`.`Pedidos Recebidos %` AS `Pedidos Recebidos % OTIF`,`o`.`Total Pedidos` AS `Total Pedidos OTIF`,`ot`.`Pedidos Recebidos` AS `Pedidos Recebidos OTD`,`ot`.`Pedidos Recebidos %` AS `Pedidos Recebidos % OTD`,`ot`.`Total Pedidos` AS `Total Pedidos OTD`,`ofr`.`Total_Pedidos` AS `Total Pedidos OFR`,`ofr`.`Pedidos_Atendidos_Completamente` AS `Pedidos Atendidos Completamente OFR`,`ofr`.`Order_Fill_Rate` AS `Order Fill Rate OFR` from (((((`tempo_entrega` `te` join `tempo_ciclo_pedido` `tcp` on((`te`.`Pedido Numero` = `tcp`.`Pedido Numero`))) join `otif` `o` on((`te`.`Pedido Numero` = `o`.`Pedido Numero`))) join `otd` `ot` on((`o`.`Cliente` = `ot`.`Cliente`))) join `orderfillrateresult` `ofr` on((`o`.`Cliente` = `ofr`.`Cliente`))) join `ofr_sep_exp` `ofrse` on((`te`.`Pedido Numero` = `ofrse`.`Pedido Numero`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-01-18 20:01:54
